...
v0.3.2 (agent-facing explainability + onboarding): Phase 9.5 shipped the **Explainable-AI axis** (ADR-009) — a grounded self-explanation + **explanation auditor** for external coding AIs ("the AI is a witness, the layer is the notary"), validated by a seeded battery where an opencode agent (big-pickle caliber) authored the narrations: **3/3 faithful PASS at groundedness 1.0, 1/1 fabricated reference caught**. Phase 10 groundwork then closed its two data-driven open items: a **deleted-symbols view** (structural snapshot diffed across builds; the auditor cites removed symbols as `deleted`, never `invented`) and a **threshold-from-data sweep** (safe band `(0.667, 1.0]`, default 0.8 kept in band). Onboarding for AI agents is now first-class: `HowToUse.md`, an `AI LAYER` guide, and a bundled opencode skill (Sections 13, 15).
...
6. **Validation.** Phase 9 executed: the **draft automated study** (baseline 0.15 / SSRL-grounded 0.61 / deterministic layer 0.32, entity-F1 vs facts gold) and a **refusal probe battery** proving the micro model's refusals were a prompt artifact — the CLI-contract + evidence-filtering improvement is exactly the context-system fix it motivated (REPORT-P9 §6.1–§6.2). ✅ groundwork. Phase 9.5 extended the surface to **external coding AIs** (ADR-009): a grounded self-explanation contract + explanation auditor, validated by a seeded battery (3/3 faithful PASS, 1/1 fabricated reference caught). Phase 10 groundwork as of v0.3.2: **deleted-symbols view** (structural snapshots across builds; auditor `deleted` citations) and a **threshold-from-data sweep** (safe band `(0.667, 1.0]`, default 0.8 in band), plus agent onboarding artifacts (`HowToUse.md`, `AI LAYER/`, bundled opencode skill). The **definitive human-graded multi-arm study** (accuracy + time-to-understand, mid-size model arm, closed tasks) is the remaining step, scoped for publication (Phase 10). ⏳
...
v0.3.2 additions to the prototype of record: a **deleted-symbols view** (`ssrl/history.py` — structural, facts-only snapshots diffed across cached builds, surfaced as CLI/MCP `deleted`; the auditor's `deleted` citation kind makes narrations about removals audit-able instead of missing-module noise), a **data-driven threshold check** (`lab/p10_eai.py` sweeps `pass_groundedness` → safe band `(0.667, 1.0]`, default 0.8 kept in band: a 4-row battery is too small to move it), and **agent onboarding** (`HowToUse.md`, `AI LAYER/` guide, bundled opencode skill). Suite: **104 tests green**, deterministic, stdlib-only.
